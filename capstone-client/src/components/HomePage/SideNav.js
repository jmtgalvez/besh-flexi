...
const NavLink = ({ pageTo }) => {
  const {activePage, setActivePage} = useContext(PageContext);

  return (
    <a onClick={ () => setActivePage(pageTo.toUpperCase()) } className={ activePage == pageTo.toUpperCase() ? 'sidenav-active' : ''} >
      { pageTo == 'Home' ? <HomeSvg /> :
        pageTo == 'Trending' ? <TrendingSvg /> :
        pageTo == 'Chat' ? <ChatSvg /> :
